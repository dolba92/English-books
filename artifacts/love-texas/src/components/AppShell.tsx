...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      className={`app-shell ${isReader ? 'reader-shell' : ''} paper-grain min-h-[100dvh] flex flex-col md:flex-row bg-background transition-colors duration-300`}
    ...
    >
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`group flex min-w-[64px] flex-1 md:flex-none items-center justify-center md:justify-start gap-3 px-2 md:px-3 py-2.5 rounded-xl transition-all duration-200 ${
                    isActive
                      ? 'bg-sidebar-primary text-sidebar-primary-foreground shadow-[0_8px_18px_hsl(var(--sidebar-primary)/.2)]'
      ...
      <main
        className="flex-1 w-full min-h-full max-w-full overflow-x-hidden relative"
        style={customBackgroundColor ? { backgroundColor: customBackgroundColor } : undefined}
      >
        {children}
      </main>
